...
// Foto do item do catálogo — mesmo padrão de AnexarFotoCatalogoEpiCommand/
// AnexarFotoCatalogoUniformeCommand: binário guardado direto na linha do CatalogoEpc, upload
...
public class AnexarFotoCatalogoEpcCommandHandler : IRequestHandler<AnexarFotoCatalogoEpcCommand>
{
    ...
    public async Task Handle(AnexarFotoCatalogoEpcCommand request, CancellationToken ct)
    {
        var item = await _db.CatalogoEpcs.FirstOrDefaultAsync(x => x.Id == request.CatalogoEpcId, ct)
            ?? throw new KeyNotFoundException($"Item de catálogo de EPC {request.CatalogoEpcId} não encontrado.");
