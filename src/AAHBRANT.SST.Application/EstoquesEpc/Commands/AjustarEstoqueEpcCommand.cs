...
// Correção manual de estoque numa Obra — recebe o saldo final desejado (não um delta); o handler
// calcula a diferença e registra a movimentação com o delta com sinal. Observação é obrigatória,
// mesmo padrão de AjustarEstoqueUniformeCommand.
...
public class AjustarEstoqueEpcCommandHandler : IRequestHandler<AjustarEstoqueEpcCommand>
{
    ...
    public async Task Handle(AjustarEstoqueEpcCommand request, CancellationToken ct)
    {
        if (!await _db.CatalogoEpcs.AnyAsync(c => c.Id == request.CatalogoEpcId, ct))
            throw new KeyNotFoundException($"Item de EPC {request.CatalogoEpcId} não encontrado.");
