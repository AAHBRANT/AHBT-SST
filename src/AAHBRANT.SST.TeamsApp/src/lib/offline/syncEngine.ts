...
export async function syncFetchJson<T>(path: string, init?: RequestInit): Promise<T> {
  try {
...
export async function syncFetchBlob(path: string): Promise<Blob> {
  try {
    const response = await fetch(`${API_BASE_URL}${path}`);
    marcarOnline(true);
    if (!response.ok) {
...
export async function syncMutateMultipart<T>(path: string, formData: FormData): Promise<T> {
  const idempotencyKey = crypto.randomUUID();
      ...
      try {
        const response = await fetch(`${API_BASE_URL}${item.url}`, {
          ...
          headers: item.isFormData
            ...
            : { 'Content-Type': 'application/json', 'Idempotency-Key': item.idempotencyKey },
