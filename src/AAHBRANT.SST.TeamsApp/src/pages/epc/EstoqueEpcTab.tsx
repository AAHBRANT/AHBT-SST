...
// Estoque de EPC segmentado por Obra apenas (sem Tamanho, diferente de EstoqueUniformeTab.tsx) —
// mesmo padrão de EstoqueTab.tsx (EPI). Entrada manual (reposição) e ajuste (correção de
// inventário, com observação obrigatória); saídas por entrega são automáticas.
export function EstoqueEpcTab() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [listaObras, listaItens] = await Promise.all([api.obras.listar(), api.catalogosEpc.listar()]);
        setObras(listaObras);
        setItens(listaItens);
